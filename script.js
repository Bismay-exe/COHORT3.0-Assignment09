...
function renderTodos() {
    ...
    todos.forEach((t, i) => {
        ...
        let textStyle = t.done ? "text-decoration: line-through; color: gray; font-style: italic;" : "";
        let checkedAttr = t.done ? "checked" : "";
        let titleHtml = t.title;
        if (t.important) {
            titleHtml += " <span class='text-[10px] bg-red-500 text-white px-2 py-0.5 rounded ml-2 align-middle font-bold tracking-wider'>IMP</span>";
        }
        let descHtml = t.desc !== "" ? `<p class='text-(--text-muted) font-medium mt-1 break-all' style='${textStyle}'>${t.desc}</p>` : "";
